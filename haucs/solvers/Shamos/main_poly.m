...
%     numpad = 5;
%     paddedpath = zeros(length(optimal_path) * numpad, 2);
%     for k=1:length(optimal_path)-1
%         x1 = optimal_path(k,1);
%         y1 = optimal_path(k,2);
%         x2 = optimal_path(k+1,1);
%         y2 = optimal_path(k+1,2);
%         paddedx = linspace(x1,x2,numpad);
%         paddedy = linspace(y1,y2,numpad);
%         paddedpath((numpad*k-(numpad-1)):(numpad*k),:)=[paddedx.',paddedy.'];
%     end
    optimal_pathc = optimal_path(2:end-1,:);%do not include depot start and end
%     nodegroup = [p_groups{i}.x,p_groups{i}.y];
