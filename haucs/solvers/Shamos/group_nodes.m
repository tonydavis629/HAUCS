...
%     pts_clus = ceil(length(nodes)/numdiv);
    ...
    %fails on 6. needs 2 loops. outer is nodes, inner is clusters. run
    %inner loop to assign individual nodes to cluster until all nodes are
    %done
...
%         k{i} = boundary(nodes(idx==i,1),nodes(idx==i,2),1);
        k{i} = convhull(nodes(idx==i,1),nodes(idx==i,2));
